/****************************************************************
Program to compute the run of ionization parameter versus radius
(measured in rg) at a specific Eddington ratio and spin due to illumination
...
double H(double loglambda);
void zroots(fcomplex a[], int m, fcomplex roots[], int polish);

int main(int argc, char *argv[]) {
  FILE *outputfp;
  char hnam[80], spinnam[80], eddnam[80], outfnam[80]="xiLedd_h";
  double height, spin, radius;
  double logrmin, logrmax, lograd, dlogr;
  double integral, rfac, rmin;
  double y,x,Rr, Rt, Rz;
  double A,B,C,E,L,temp1,temp2,temp3,temp4;
  double lambda, loglambda;
  double xi,y1,y2,y3,yms;
  fcomplex b1[4],roots2[3];
  int k;
  
  if(argc < 4) {
    fprintf(stderr, "Usage: height [rg], spin, Eddington ratio\n");
    exit (1);
  }
  ...
  sscanf(argv[3],"%s",eddnam);

  strcat(outfnam,hnam);
  strcat(outfnam,"_a");
  strcat(outfnam,spinnam);
  strcat(outfnam,"_edd");
  strcat(outfnam,eddnam);
  ...
  for (k=0; k <= NSTEP; k++) {
    lograd=logrmin+k*dlogr;
    radius=pow(10.0,lograd);
    /* Calculate the Rr, Rz, and Rt factors for this radius and spin */
    x=radius;
    if (x < 4.0) {
      x=4.0; 
    } 
    ...
    xi=(5.44e10)*pow((ETA/0.1),-2)*(ALPHA/0.1)*pow(lambda,3.0)*pow(radius,-1.5)*pow(Rz,-2)*pow(Rr,3)*F(loglambda)*pow((1-F(loglambda)),3)*FKFitFormula(radius,height)*glp(radius,spin,height)*glp(radius,spin,height)/(Rt*integral);
